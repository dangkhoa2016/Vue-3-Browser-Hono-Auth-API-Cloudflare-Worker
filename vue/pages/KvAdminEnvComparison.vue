      ...
      >
          ...
          <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-100/50 dark:bg-indigo-500/10 border border-indigo-200/50 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-300 text-xs font-semibold tracking-wide uppercase mb-3">
              <i class="bi bi-arrow-left-right"></i>
               {{ tf('message.kv_admin_page.env_comparison.badge', 'KV vs ENV') }}
            </div>
            <h1 class="text-3xl lg:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">{{ tf('message.kv_admin_page.env_comparison.title', 'Environment Comparison') }}</h1>
        ...
        <div v-if="!isLoading && summary.total" class="relative mt-8 p-4 rounded-2xl bg-white/60 dark:bg-slate-800/60 border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm flex flex-wrap gap-4 items-center">
             ...
             <button @click="sourceFilter = 'kv'" :class="['flex flex-col px-4 border-r border-slate-200 dark:border-slate-700 hover:opacity-80 transition cursor-pointer', sourceFilter === 'kv' ? 'bg-emerald-50 dark:bg-emerald-900/30 rounded-lg py-1' : 'py-1']">
               <span class="text-xs text-emerald-600 dark:text-emerald-400 font-medium uppercase tracking-wider">{{ tf('message.kv_admin_page.env_comparison.stats_from_kv', 'From KV') }}</span>
             ...
             <button @click="sourceFilter = 'env'" :class="['flex flex-col px-4 border-r border-slate-200 dark:border-slate-700 hover:opacity-80 transition cursor-pointer', sourceFilter === 'env' ? 'bg-indigo-50 dark:bg-indigo-900/30 rounded-lg py-1' : 'py-1']">
               <span class="text-xs text-indigo-600 dark:text-indigo-400 font-medium uppercase tracking-wider">{{ tf('message.kv_admin_page.env_comparison.stats_from_env', 'From ENV') }}</span>
             ...
             <button @click="sourceFilter = 'default'" :class="['flex flex-col px-4 hover:opacity-80 transition cursor-pointer', sourceFilter === 'default' ? 'bg-slate-100 dark:bg-slate-700 rounded-lg py-1' : 'py-1']">
               <span class="text-xs text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">{{ tf('message.kv_admin_page.env_comparison.stats_from_default', 'From Default') }}</span>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="relative w-full max-w-md">
          ...
          <input 
            v-model="search" 
            type="text" 
            :placeholder="tf('message.kv_admin_page.search_placeholder', 'Filter keys...')" 
        ...
        <div v-if="sourceFilter !== 'all' || search" class="flex flex-wrap gap-2 items-center text-sm">
          <span class="text-slate-500 dark:text-slate-400"><i class="bi bi-funnel"></i> {{ tf('message.kv_admin_page.env_comparison.active_filters', 'Active filters:') }}</span>
          ...
          <div v-if="sourceFilter !== 'all'" class="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm">
            <span class="text-xs font-semibold uppercase">{{ tf('message.kv_admin_page.env_comparison.source', 'Source') }}: 
              <template v-if="sourceFilter === 'kv'">{{ tf('message.kv_admin_page.source_kv', 'KV') }}</template>
              <template v-else-if="sourceFilter === 'env'">{{ tf('message.kv_admin_page.env_comparison.stats_from_env', 'ENV') }}</template>
              <template v-else-if="sourceFilter === 'default'">{{ tf('message.kv_admin_page.env_comparison.default', 'DEFAULT') }}</template>
          ...
          <button v-if="(sourceFilter !== 'all' && search) || sourceFilter !== 'all' || search" @click="sourceFilter = 'all'; search = ''" class="text-xs text-rose-500 hover:text-rose-600 dark:text-rose-400 hover:underline px-2 cursor-pointer transition">
             {{ tf('message.kv_admin_page.env_comparison.clear_all_filters', 'Clear all filters') }}
          </button>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-900 rounded-[28px] border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6 p-4 lg:p-6 bg-slate-50/50 dark:bg-slate-900/50">
          <div v-for="key in filteredKeys" :key="key" class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col">
            <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/80 flex items-center justify-between gap-4">
              ...
              <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shrink-0" :class="sourceBadgeClass(comparison[key].source)">
                <template v-if="comparison[key].source === 'kv'">{{ tf('message.kv_admin_page.source_kv', 'KV') }}</template>
                <template v-else-if="comparison[key].source === 'env'">{{ tf('message.kv_admin_page.env_comparison.stats_from_env', 'ENV') }}</template>
                <template v-else-if="comparison[key].source === 'default'">{{ tf('message.kv_admin_page.source_default', 'DEFAULT') }}</template>
                <template v-else>{{ comparison[key].source }}</template>
              </span>
            </div>
            <div class="p-4 grid grid-cols-1 gap-4 flex-1">
              <div class="flex flex-col gap-1.5">
                <span class="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ tf('message.kv_admin_page.env_comparison.column_kv_value', 'KV Value') }}</span>
              ...
              <div class="flex flex-col gap-1.5">
                <span class="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ tf('message.kv_admin_page.env_comparison.column_env_value', 'ENV Value') }}</span>
                <div class="bg-slate-50 dark:bg-slate-900/40 rounded-xl p-3 border border-slate-100 dark:border-slate-800/80">
          ...
          <div v-if="filteredKeys.length === 0" class="col-span-full py-16 text-center text-slate-500 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
            ...
            {{ tf('message.kv_admin_page.env_comparison.search_empty', 'No configurations matched your search') }} <span class="font-semibold text-slate-700 dark:text-slate-300">"{{ search }}"</span>
          </div>
        </div>
      </div>
    </template>
...
import { useKvAdminEnvComparisonPage } from '/vue/composables/useKvAdminEnvComparisonPage.js';
